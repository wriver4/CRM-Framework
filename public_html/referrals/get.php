...
if (isset($_GET['action'])) {
    switch ($_GET['action']) {
        case 'referral_id':
            // Get the last referral ID
            $last_referral_id = $leads->get_last_lead_id();
            echo $last_referral_id;
            break;
            
        case 'referral_with_user':
            // Get referral by ID with user information
            if (isset($_GET['id'])) {
                $referral = $leads->get_lead_by_lead_id($_GET['id']);  // Use lead_id (external number) instead of internal id
                if ($referral && !empty($referral['last_edited_by'])) {
                    $referral['last_edited_by_name'] = $users->get_name_by_id($referral['last_edited_by']);
                }
        ...
        default:
            // Default: Get the last referral ID
            $last_referral_id = $leads->get_last_lead_id();
            echo $last_referral_id;
            break;
    }
} else {
    // Default: Get the last referral ID for backward compatibility
    $last_referral_id = $leads->get_last_lead_id();
}
...
if ($dir == 'referrals' && $page == 'edit') {
    ...
    // Get contacts associated with this referral for the contact dropdown in notes
