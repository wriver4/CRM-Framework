...
if (isset($_GET['action'])) {
    switch ($_GET['action']) {
        case 'prospect_id':
            // Get the last prospect ID
            $last_prospect_id = $leads->get_last_lead_id();
            echo $last_prospect_id;
            break;
            
        case 'prospect_with_user':
            // Get prospect by ID with user information
            if (isset($_GET['id'])) {
                $prospect = $leads->get_lead_by_lead_id($_GET['id']);  // Use lead_id (external number) instead of internal id
                if ($prospect && !empty($prospect['last_edited_by'])) {
                    $prospect['last_edited_by_name'] = $users->get_name_by_id($prospect['last_edited_by']);
                }
        ...
        default:
            // Default: Get the last prospect ID
            $last_prospect_id = $leads->get_last_lead_id();
            echo $last_prospect_id;
            break;
    }
} else {
    // Default: Get the last prospect ID for backward compatibility
    $last_prospect_id = $leads->get_last_lead_id();
}
...
if ($dir == 'prospects' && $page == 'edit') {
    ...
    // Get contacts associated with this prospect for the contact dropdown in notes
