...
-- Update any other tables that reference contacts.id to match
-- Check if leads_contacts table exists and update it
SET @table_exists = (
  SELECT COUNT(*)
...
SET @sql = IF(@table_exists > 0,
  ...
  'SELECT "leads_contacts table does not exist, skipping" as message'
);
...
SET @sql = IF(@table_exists > 0,
  ...
  'SELECT "users table does not exist, skipping" as message'
);
...
SET @sql = IF(@table_exists > 0,
  ...
  'SELECT "roles table does not exist, skipping" as message'
);
...
-- Show the standardized column types for verification
SELECT 
  TABLE_NAME,
  COLUMN_NAME,
  COLUMN_TYPE,
...
WHERE TABLE_SCHEMA = DATABASE()
  AND COLUMN_NAME = 'id'
  AND TABLE_NAME IN ('leads', 'contacts', 'users', 'roles', 'phplist_subscribers', 'phplist_config', 'phplist_sync_log')
ORDER BY TABLE_NAME;