-- Email Processing System Tables Migration
...
-- Add foreign key constraint to leads table (if exists)
...
SET @constraint_exists = (SELECT COUNT(*) FROM information_schema.KEY_COLUMN_USAGE 
    WHERE TABLE_SCHEMA = 'democrm_democrm' 
    AND TABLE_NAME = 'email_form_processing' 
    AND CONSTRAINT_NAME = 'fk_email_processing_lead_id');

SET @sql = IF(@constraint_exists = 0, 
    'ALTER TABLE `email_form_processing` ADD CONSTRAINT `fk_email_processing_lead_id` FOREIGN KEY (`lead_id`) REFERENCES `leads` (`id`) ON DELETE SET NULL',
...
-- Only add if the constraint doesn't already exist
SET @constraint_exists2 = (SELECT COUNT(*) FROM information_schema.KEY_COLUMN_USAGE 
    WHERE TABLE_SCHEMA = 'democrm_democrm' 
    AND TABLE_NAME = 'crm_sync_queue' 
    AND CONSTRAINT_NAME = 'fk_crm_sync_lead_id');

SET @sql2 = IF(@constraint_exists2 = 0, 
    'ALTER TABLE `crm_sync_queue` ADD CONSTRAINT `fk_crm_sync_lead_id` FOREIGN KEY (`lead_id`) REFERENCES `leads` (`id`) ON DELETE CASCADE',
    'SELECT "Foreign key constraint already exists" as message');
...
-- ========================================
-- 6. CLEANUP QUERIES (for testing/rollback)
-- ========================================
