...
-- Step 1: Standardize existing table ID fields to prevent future issues
-- This ensures all foreign key relationships will work properly

-- First, let's check and fix the contacts table ID field
...
-- Step 2: Ensure all other critical ID fields are standardized
-- Note: Using conditional logic without information_schema queries

-- Standardize users table if it exists (will fail silently if table doesn't exist)
-- We'll use a different approach that doesn't rely on information_schema
SET @sql = 'ALTER TABLE users MODIFY COLUMN id int(11) NOT NULL AUTO_INCREMENT';
SET @error = 0;

-- Try to execute the statement, ignore errors if table doesn't exist
BEGIN;
  -- This will work if users table exists, fail silently if not
  SET @sql_safe = CONCAT('SELECT COUNT(*) FROM users LIMIT 1');
  PREPARE stmt_check FROM @sql_safe;
  EXECUTE stmt_check;
...
SET @sql = 'ALTER TABLE roles MODIFY COLUMN id int(11) NOT NULL AUTO_INCREMENT';

BEGIN;
  -- Check if roles table exists by trying to query it
  SET @sql_safe = CONCAT('SELECT COUNT(*) FROM roles LIMIT 1');
  PREPARE stmt_check FROM @sql_safe;
  EXECUTE stmt_check;
  DEALLOCATE PREPARE stmt_check;
...
-- Step 3: Create phpList subscribers table with standardized field types
...
-- Step 4: Insert default phpList configuration
...
-- Step 5: Add foreign key constraints AFTER all tables are standardized
-- Now all ID fields are int(11) so foreign keys will work properly
-- Note: We'll add these without checking existence since they'll fail gracefully if they exist
...
-- Step 6: Verification queries to confirm everything is properly standardized
