...
-- FINAL FIX: Handles data type mismatches between tables

-- Create phpList subscribers table (WITHOUT foreign keys initially)
-- CRITICAL: Match exact data types from existing tables
CREATE TABLE IF NOT EXISTS `phplist_subscribers` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `lead_id` int(11) NOT NULL COMMENT 'Foreign key to leads table (matches leads.id type)',
  `contact_id` int(10) UNSIGNED DEFAULT NULL COMMENT 'Foreign key to contacts table (matches contacts.id type)',
...
-- CRITICAL: contacts.id is int(10) UNSIGNED, so our contact_id must match
SET @constraint_exists = (
  SELECT COUNT(*)
  FROM information_schema.TABLE_CONSTRAINTS 
  WHERE CONSTRAINT_SCHEMA = DATABASE()
...
-- Success message
SELECT 'phpList tables created successfully with proper data type matching!' as result;