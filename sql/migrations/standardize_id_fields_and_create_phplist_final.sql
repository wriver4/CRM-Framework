...
-- FIXED: Handles existing foreign key constraints that prevent column modifications
-- FIXED: Removed all information_schema queries to prevent #1044 errors

-- Step 1: Drop existing foreign key constraints that reference contacts.id
-- This allows us to modify the contacts.id column type

-- Drop foreign key from leads table if it exists
SET @sql = 'ALTER TABLE leads DROP FOREIGN KEY fk_leads_contact_id';
SET @sql_safe = CONCAT('SELECT 1');
...
BEGIN;
  ...
  SET @sql_exec = 'ALTER TABLE leads DROP FOREIGN KEY fk_leads_contact_id';
  PREPARE stmt FROM @sql_exec;
  EXECUTE stmt;
  DEALLOCATE PREPARE stmt;
COMMIT;

-- Drop foreign key from leads_contacts table if it exists
BEGIN;
  -- Check if leads_contacts table exists first
  SELECT COUNT(*) FROM leads_contacts LIMIT 1;
  
  -- Try to drop the foreign key constraint
  SET @sql_exec = 'ALTER TABLE leads_contacts DROP FOREIGN KEY fk_leads_contacts_contact_id';
  PREPARE stmt FROM @sql_exec;
  EXECUTE stmt;
  DEALLOCATE PREPARE stmt;
COMMIT;

-- Step 2: Standardize existing table ID fields to prevent future issues
-- This ensures all foreign key relationships will work properly

-- Change contacts.id from int(10) UNSIGNED to int(11) NOT NULL
-- This should now work since we dropped the foreign key constraints
ALTER TABLE `contacts` MODIFY COLUMN `id` int(11) NOT NULL AUTO_INCREMENT;

-- Update leads.contact_id to match the new contacts.id type
ALTER TABLE `leads` MODIFY COLUMN `contact_id` int(11) DEFAULT NULL;

-- Update leads_contacts.contact_id to match if the table exists
BEGIN;
  -- Check if leads_contacts table exists
  SELECT COUNT(*) FROM leads_contacts LIMIT 1;
  
  -- Update the contact_id column type
  ALTER TABLE `leads_contacts` MODIFY COLUMN `contact_id` int(11) NOT NULL;
COMMIT;

-- Step 3: Recreate the foreign key constraints with proper types
-- Now that all columns are int(11), the constraints should work

-- Recreate foreign key for leads.contact_id -> contacts.id
BEGIN;
...
-- Recreate foreign key for leads_contacts.contact_id -> contacts.id
BEGIN;
  -- Check if leads_contacts table exists
  SELECT COUNT(*) FROM leads_contacts LIMIT 1;
  
  -- Add the foreign key constraint back
  ALTER TABLE leads_contacts ADD CONSTRAINT fk_leads_contacts_contact_id FOREIGN KEY (contact_id) REFERENCES contacts(id) ON DELETE CASCADE;
COMMIT;

-- Step 4: Standardize other critical ID fields if tables exist
...
BEGIN;
  ...
  -- If we get here, table exists, so modify it
...
-- Step 5: Create phpList subscribers table with standardized field types
...
-- Step 6: Insert default phpList configuration
...
-- Step 7: Add foreign key constraints for phpList tables AFTER all tables are standardized
-- Now all ID fields are int(11) so foreign keys will work properly
...
-- Step 8: Verification queries to confirm everything is properly standardized
...
-- Show the standardized column types for verification using DESCRIBE instead of information_schema
SELECT 'Verifying contacts table structure:' as verification;
DESCRIBE contacts;

SELECT 'Verifying leads table structure:' as verification;  
DESCRIBE leads;

SELECT 'Verifying phplist_subscribers table structure:' as verification;
DESCRIBE phplist_subscribers;